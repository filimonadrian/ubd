...
desc dba_segments;
select owner,segment_name,segment_type, tablespace_name, blocks, extents from dba_segments where owner='SCOTT'and segment_type='TABLE';


-- 4) Informatii despre dimensiunea extensiilor alocate unui segment:
desc dba_extents
select owner, segment_name, segment_type, tablespace_name from dba_extents where owner='SCOTT' and segment_name='EMP';
select segment_name, extent_id, file_id, block_id, blocks, bytes from dba_extents where owner='SCOTT' and segment_name='EMP';
